...
class Confirm(QDialog):
    def __init__(self, food_app, parent=None):
        ...
        print(self.food_app.order_table.rowCount())
        self.message = ""
    ...
    def confirm_action(self):
        ...
        try:
            ...
            cgst = sgst = (sum(self.total) * 0.05) / 2
            discount = round((sum(self.total) * 0.07))
            total_gst = cgst + sgst

            self.total_qty = sum(self.qty)

            # Total bill amount
            total_amt = sum(self.total) + total_gst - discount

            # Line edit update
            self.total_le.setText(str(sum(self.total)))
            self.cgst_sgst_lbl.setText(str(total_gst))
            self.disc_lbl.setText(str(discount))
            self.ord_lbl.setText(str(self.id))
            self.itemsSold_le.setText(str(sum(self.total_qty)))
            self.date_le.setText(str(self.formatted_datetime))
            self.total_bill_le.setText(str(total_amt))
        ...
        try:
            ...
            for i in range(self.table2.rowCount()):
                sql_query = "INSERT INTO food_txn VALUES ('{}', '{}', '{}', '{}', '{}', '{}')".format(
                    str(self.id), str(self.name[i]), str(self.qty[i]), str(self.pri[i]), str(self.total[i]),
...
class FoodApp(QMainWindow):
    def __init__(self):
        ...
        self.order_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeToContents)  # Resize columns to contents
    ...
    def remove_row(self, row):
        print(row)
        try:
            self.order_table.item(row, 0).text()
            price = float(self.order_table.item(row, 1).text())
            quantity = float(self.quantity_spinbox.text())

            # Update total price after removing a row
    ...
    def add_to_order(self, item):
        ...
        for row in range(self.order_table.rowCount()):
            ...
            self.order_table.setCellWidget(row, 4, self.remove_button)  # Place the button in the "Remove" column
            print("removing a row from order")
            self.remove_button.clicked.connect(
                lambda _, i=row: self.remove_row(i))  # Connect the button's clicked signal
            self.row = row
        # SpinBox for the quantity column
        self.quantity_spinbox = QSpinBox()
        self.quantity_spinbox.setMinimum(1)
        self.quantity_spinbox.setMaximum(100)

        row_index = self.row
        self.order_table.setCellWidget(row_index, 2, self.quantity_spinbox)

        # Adjust column widths
        self.order_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)  # Item column
        self.order_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents)  # Price column
        self.order_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents)  # Quantity column
    ...
    def proceed_to_confirm(self):
        ...
        for row in range(self.order_table.rowCount()):
            ...
            quantity = int(self.quantity_spinbox.text())
            price = float(self.order_table.item(row, 1).text())
